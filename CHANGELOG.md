...
### Added
- **`skills/codex-cli-rules/`** — new skill covering operational rules for driving Codex CLI from scripts: stdin-vs-argv (`ARG_MAX` ~1MB on macOS), three-element success contract (`out > 500B` + log has `tokens used` + `.exit == 0`), three-dot diff (`BASE...HEAD` walks merge-base, two-dot includes BASE-side commits), and worktree `-C` flag (without it `codex exec` reads main repo HEAD not worktree). Distilled from real wrapper-script failure modes; placeholders only, no private session refs. Slots into the "Code quality" category.
- **`skills/finishing-a-development-branch/SKILL.md`** — new `## Rescue: leaked secret` section. ~10-line worked example for `git filter-repo --email-callback` recipe (with placeholders), plus three operational gotchas: filter-repo strips origin remote on purpose (re-add after), paths-filtered CI doesn't trigger on metadata-only force-push (add `workflow_dispatch:`), and rotate-the-credential-anyway because public mirrors retain content. Explicit framing: rewriting public history is irreversible from your end.
- **README "Start here"** — 3 new rows: drive Codex CLI from a script (→ `codex-cli-rules`), Figma MCP ran out of View seat quota (→ existing `figma-playwright-fallback`), accidentally pushed private email/API key to public repo (→ `finishing-a-development-branch#rescue-leaked-secret`). README "Code quality" category bumped from 1 to 2 skills. Skills count badge bumped 66 → 67.
