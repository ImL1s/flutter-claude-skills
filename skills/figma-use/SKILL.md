...
**If the task involves building or updating a full page, screen, or multi-section layout in Figma from code**, this skill covers the API rules. For the screen-building workflow itself — discovering design system components via `search_design_system`, importing them, and assembling screens incrementally — drive the Figma MCP tools directly (`search_design_system`, `get_libraries`, `use_figma` for batched node creation).
...
- **`figma-implement-design`** — **DISAMBIGUATION**: figma-use extracts design context from Figma (tokens, intent, structure). figma-implement-design translates that context into code. Use figma-use to understand the design; use figma-implement-design to build it.
- **`verify-ui`** → **`visual-verdict`** → **`verify-ui-auto`** — after implementation, compare screenshots against reference design, quantify differences, and set up automated regression testing.
