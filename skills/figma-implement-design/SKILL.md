...
- **`figma-use`** — **DISAMBIGUATION**: figma-use extracts design context from Figma (tokens, intent, structure). figma-implement-design translates that context into code. Use figma-use first to understand the design; then use figma-implement-design to build it.
- **`verify-ui`** — after implementing, compare screenshots against the Figma reference to identify layout, spacing, and typography mismatches.
- **`visual-verdict`** — quantify the diff between your implementation and the reference using image-based diffing.
- **`verify-ui-auto`** — set up automated golden tests after manual verification passes, preventing future visual regressions.
